...
public class ReplaceGetWithBracketsIntention() : PsiElementBaseIntentionAction() {
    ...
    override fun isAvailable(project: Project, editor: Editor, element: PsiElement): Boolean {
        ...
        // ... and:
        // - there is at least one non-named argument
        // - there are no type arguments, named arguments or function literal arguments (either as positional or trailing parameters)
        if (callExpr.getValueArguments().isEmpty())
            return false
        if (callExpr.getValueArguments().any { it!!.isNamed() || it.getArgumentExpression() is JetFunctionLiteral })
            return false
        ...
        val arrayExpr = createReplacement(project, dotExpr, callExpr)
        setText(JetBundle.message("replace.get.with.brackets", dotExpr.getText(), arrayExpr.getText()))
        return true
    }
